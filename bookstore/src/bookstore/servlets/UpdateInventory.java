...
import org.mcgill.ccs2_505.assignment03.inventory.Book;
import org.mcgill.ccs2_505.assignment03.inventory.CompactDisc;
import org.mcgill.ccs2_505.assignment03.inventory.Inventory;
import org.mcgill.ccs2_505.assignment03.inventory.InventoryEntry;
...
public class UpdateInventory extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(bookIE != null && cdIE != null) {
			...
			if ( session.getAttribute("inventoryChange") != null)
				session.removeAttribute("inventoryChange");
			session.setAttribute("inventoryChange",userName + "," + bookIE.toString());
			inventory.add(cdIE);
			if ( session.getAttribute("inventoryChange") != null)
				session.removeAttribute("inventoryChange");
			session.setAttribute("inventoryChange",userName + "," + cdIE.toString());
		}
		else if(bookIE != null) {
			inventory.add(bookIE);
